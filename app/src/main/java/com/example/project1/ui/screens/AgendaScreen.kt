...
// Importa las dependencias necesarias para el manejo de permisos, contactos, calendario y la interfaz de usuario.
...
@Composable
fun CalendarAPIScreen(navController: NavController) {
    ...
    if (hasPermission) {
        AgendaScreen() // Llama a la función que contiene la lógica principal de la pantalla
    } else {
...
fun getCalendarId(context: Context): Long? {
    ...
    cursor?.use {
        ...
        while (it.moveToNext()) {
            ...
            Log.d("CalendarDebug", "Encontrado calendario: $accountName")
            return calendarId // Devuelve el primer calendario encontrado
        }
    }

    Log.w("CalendarDebug", "No se encontró ningún calendario.")
    return null
}
