...
import com.example.project1.data.model.model.toServiceEntity
import com.example.project1.data.model.model.toServiceEntityList
...
class ServiceViewModel : ViewModel() {
    ...
    fun getServiceById(serviceId: Int, onResult: (Response<ServiceModel>) -> Unit) {
        viewModelScope.launch {
            try {
                val response =
                    api.getServiceById(serviceId) // Llamada a la API para obtener el servicio por ID
                onResult(response)
            } catch (exception: Exception) {
